...
The refreshed `gh-pages` commit contains the new four-action title deck. The first live check reached the older cached deployment, which still exposed only the original two hero actions. The published branch is correct; a final deployment-status and cache-bypassed verification is required before closing the refresh.
